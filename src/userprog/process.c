...
start_process (void *file_name_)
{
  ...
  char *command, *token, *save_ptr;
  char **parsed_token;
  int arg_cnt = 0;
  struct intr_frame if_;
  bool success;

  command = (char *)malloc(strlen(file_name) + 1);
  ...
  parsed_token = (char **)malloc(arg_cnt * sizeof(char *));
  strlcpy (command, file_name, strlen(file_name) + 1);
  token = command;
  arg_cnt = 0;
  ...
  thread_current()->is_loaded = thread_current()->tid;

  save_arg(parsed_token, arg_cnt, &if_.esp);
...
void save_arg (char **parse, int count, void **esp)
{
  ...
  *(char **)(*esp) = 0;
  ...
  for (i = count - 1; i > -1; i--) {
    *esp = *esp - sizeof(char *);
    arg_base = arg_base - (strlen(parse[i]) + 1);
    *(char **)(*esp) = arg_base;
  }
...
struct thread* get_child_process (int pid) {
  ...
  for (e = list_begin(&(t->child_list)); e != list_end(&(t->child_list)); e = list_next(e)) {
    if ((child = list_entry(e, struct thread, child_elem))->tid == pid) {
      return child;
    }
  }
  return NULL;
